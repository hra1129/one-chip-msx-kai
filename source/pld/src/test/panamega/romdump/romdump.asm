...
save_8kb::
	...
	ld			b, 8192 / 128
	ld			hl, 0x4000				; Source Address
loop:
	push		bc

	ld			de, 0x0080				; Destination Addres (DTA)s
	ld			bc, 0x0080				; Transfer size (DTA size)
	ldir

	push		hl
	call		save_one_block_from_fcb
	pop			hl
	pop			bc
	djnz		loop
...
create_fcb::
		...
		; open target file
		ld		de, fcb
		ld		c, BDOS_FUNC_FCB_CREATE_FILE
		call	BDOS_ON_MSXDOS
		inc		a
		ret		z
		; change read buffer
